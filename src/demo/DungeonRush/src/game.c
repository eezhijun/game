...
void invokeWeaponBuff(Snake *src, Weapon *weapon, Snake *dest, int damage)
{
    ...
    for (int i = BUFF_BEGIN; i < BUFF_END; i++) {
        ...
        if (random < weapon->effects[i].chance)
            ...
            case BUFF_ATTACK:
                if (src)
                    attackUpSnkae(src, weapon->effects[i].duration);
                break;
            default:
                break;
            }
    ...
    for (LinkNode *p = snake->sprites->head; p; p = p->nxt) {
        ...
        if (sprite->hp <= 0) {
            ...
            if (isPlayer(snake))
                death++;
            dropItem(sprite);
            createAndPushAnimation(&animationsList[RENDER_LIST_DEATH_ID],
                                   &textures[RES_SKULL], NULL, LOOP_INFI, 1,
                                   sprite->x + randInt(-MAP_SKULL_SPILL_RANGE,
                                                       MAP_SKULL_SPILL_RANGE),
                                   sprite->y + randInt(-MAP_SKULL_SPILL_RANGE,
                                                       MAP_SKULL_SPILL_RANGE),
                                   sprite->face == LEFT ? SDL_FLIP_NONE :
                                                          SDL_FLIP_HORIZONTAL,
                                   0, AT_BOTTOM_CENTER);
        ...
        if (snake->team != spriteSnake[i]->team) {
            for (LinkNode *p = spriteSnake[i]->sprites->head; p; p = p->nxt) {
                ...
                if (weapon->wp == WEAPON_SWORD_POINT ||
                    ...
                    Bullet *bullet = createBullet(snake, weapon, sprite->x,
