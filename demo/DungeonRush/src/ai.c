...
int balanceVerdict(Sprite *sprite, int id)
{
    if (id == -1)
        return 0;
    if (!spriteSnake[id]->sprites->head)
        return 0;
    int ret = 0;
    Sprite *player = spriteSnake[id]->sprites->head->element;
    if (player->x < sprite->x && sprite->direction == LEFT)
        ret++;
    if (player->x > sprite->x && sprite->direction == RIGHT)
        ret++;
    if (player->y > sprite->y && sprite->direction == DOWN)
        ret++;
    if (player->y < sprite->y && sprite->direction == UP)
        ret++;
    return ret;
}
